...
const ControlButtons = ({ btnEvent, activeButtons }: { btnEvent: (data: string, close: boolean) => void, activeButtons: string[] }) => {
    const obj_button: Record<string, JSX.Element> = {
        "play": <TooltipProvider key="play">
            <Tooltip>
                <TooltipTrigger>
                    <div className="w-10 h-10 p-1.5 rounded cursor-pointer hover:bg-amber-500 hover:fill-white" onClick={() => {
                        btnEvent("play", false)
                    }}>
        ...
        "pause": <TooltipProvider key="pause">
            <Tooltip>
                <TooltipTrigger>
                    <div className="w-10 h-10 rounded p-0.5 cursor-pointer hover:bg-amber-500 hover:fill-white hover:stroke-white stroke-black" onClick={() => {
                        btnEvent("play", false)
                    }}>
